...
class UserViewStat:
    def __init__(self):
        # Struktur: {viewer_user_id: {owner_user_id: total_views}}
        self.view_stats = {}
        self.lock = threading.Lock()

    def add_view(self, viewer_user_id, owner_user_id):
        with self.lock:
            ...
            self.view_stats[viewer_user_id][owner_user_id] += 1
    ...
    def get_top_viewed(self, viewer_user_id, top_n=5):
        with self.lock:
            ...
            # Mengurutkan berdasarkan jumlah views secara menurun
...
def build_user_views_from_db():
    global last_processed_time_graph
    query = """
        SELECT v.user_id AS viewer_user_id, q.user_id AS owner_user_id, v.created_at
        FROM views v
        JOIN questions q ON v.viewable_id = q.id
        WHERE v.viewable_type = 'question'
    """
    params = ()
    if last_processed_time_graph:
    ...
    for row in rows:
        ...
        user_view_stat.add_view(viewer_user_id, owner_user_id)
        last_processed_time_graph = row['created_at']  # Update waktu terakhir diproses

def monitor_views_db():
    while True:
        try:
            build_user_views_from_db()
            time.sleep(2)  # Interval polling, sesuaikan sesuai kebutuhan
        except Exception as e:
...
if __name__ == '__main__':
    ...
    threading.Thread(target=monitor_views_db, daemon=True).start()  # Thread baru
